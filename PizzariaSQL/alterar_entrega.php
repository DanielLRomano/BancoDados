...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Atualiza o registro
        $stmt = $pdo->prepare('UPDATE entregas SET id_entregas = ?, nome = ?, email = ?, cel = ?, pizza = ?, cadastro = ?, entrega = ? WHERE id_entregas = ?');
        $stmt->execute([$id_entregas, $nome, $email, $cel, $pizza, $cadastro, $entrega, $_GET['id']]);
