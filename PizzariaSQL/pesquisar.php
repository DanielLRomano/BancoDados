<?php
    ...
    if(isset($_GET['search1'])) {
        ...
        // Preparar a consulta SQL para buscar as pizzas com o sabor selecionado
        $search = '%' . $_GET['search1'] . '%';
        $stmt = $pdo->prepare('SELECT * FROM promocoes WHERE nome_promocao LIKE :search1');
...
<body>
    ...
    <div class="content read1">
            <form class="" id="searchForm" action="" method="get">
                <label for="search1">Pesquisar por promoção:</label>
                <select id="search1" name="search1">
                    ...
                    <?php
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            $selected = ($_GET['search1'] ?? '') === $row['nome_promocao'] ? 'selected' : '';
